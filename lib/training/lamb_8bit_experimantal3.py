...
class CPULAMB8Bit(Optimizer2State):
    ...
    def _update_moments_and_compute_delta(
        ...
        if state["state1"].dtype != torch.uint8:
            ...
            if torch.all(exp_avg == 0) or torch.all(exp_avg_sq == 0):
                print('exp_avg OR exp_avg_sq IS ZERO, SKIPPING `_update_moments_and_compute_delta`.')
            ...
            for chunk_index, chunk_start in enumerate(range(0, len(flat_p), chunk_size)):
                ...
                if torch.all(exp_avg_chunk == 0) or torch.all(exp_avg_sq_chunk == 0):
                    print('exp_avg_chunk OR exp_avg_sq_chunk IS ZERO, SKIPPING `_update_moments_and_compute_delta`.')
                    return torch.zeros_like(
                        p_cpu,
                        memory_format=torch.preserve_format,
                        dtype=p_cpu.dtype,
