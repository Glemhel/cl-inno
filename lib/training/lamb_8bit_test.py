...
class CPULAMB8Bit(Optimizer2State):
    ...
    def _update_moments_and_compute_delta(
        ...
        if state["state1"].dtype != torch.uint8:
            ...
            print('STATE', state["state1"], state["state1"].dtype)
            print('ABSMAX', state["absmax1"], state["absmax1"].dtype)
            print('QMAP', state["qmap1"], state["qmap1"].dtype)
            ...
            print('\nATTEMPTING QUANTIZE_BLOCKWISE RAND!!!!')
            A1 = torch.rand(16384, 4, device="cpu", dtype=torch.float32)
            quantize_blockwise(A1)
            A2 = torch.rand(4, 16384, device="cpu", dtype=torch.float32)
            quantize_blockwise(A2)
            print('\nATTEMPTING QUANTIZE_BLOCKWISE RAND LIKE STATE!!!!')
            A3 = torch.rand_like(state["state1"], memory_format=torch.preserve_format, 
                                 device="cpu", dtype=torch.float32)
            print('A3', A3, A3.dtype, A3.shape, A3.device)
            quantize_blockwise(A3)
            print('\nATTEMPTING QUANTIZE_BLOCKWISE RAND LIKE EXP_AVG!!!!')
            A4 = torch.rand_like(exp_avg, memory_format=torch.preserve_format, 
                                 device="cpu", dtype=torch.float32)
            n = A4.numel()
            blocks = n // block_size
            blocks += 1 if n % block_size > 0 else 0
            A4_absmax = torch.zeros((blocks,), dtype=torch.float32, device=A4.device)
            print('A4', A4, A4.dtype, A4.shape, A4.device)
            print('A4_absmax before', A4_absmax)
            quantize_blockwise(A4, absmax=A4_absmax)
            print('A4_absmax after', A4_absmax)

            print('\nATTEMPTING QUANTIZE_BLOCKWISE RAND FULL Q-DQ-Q!!!!')
            A5 = torch.rand_like(state["state1"], memory_format=torch.preserve_format, 
                                 device="cpu", dtype=torch.float32)
            print('A5', A5)
            n = A5.numel()
            blocks = n // block_size
            blocks += 1 if n % block_size > 0 else 0
            A5_ABSMAX = torch.zeros((blocks,), device=A5.device, dtype=torch.float32)
            A5_QMAP = state["qmap1"].clone()
            A5_QUANT = torch.zeros_like(A5, dtype=torch.uint8)

            quantize_blockwise(A5, absmax=A5_ABSMAX, code=A5_QMAP, out=A5_QUANT)
            print('A5_ABSMAX', A5_ABSMAX)
            print('A5_QUANT', A5_QUANT)

            A5_deq = dequantize_blockwise(
                A5_QUANT,
                absmax=A5_ABSMAX, code=A5_QMAP,
                blocksize=block_size,   
            )
            print('A5_deq init', A5_deq)
            A5_deq.mul_(beta1).add_(grad_cpu, alpha=1 - beta1)
            print('A5_deq updated', A5_deq)
            print('A5_deq', A5_deq, A5_deq.dtype, A5_deq.shape, A5_deq.device)
            quantize_blockwise(A5_deq, absmax=A5_ABSMAX, code=A5_QMAP)

            print('\nATTEMPTING QUANTIZE_BLOCKWISE RAND FULL DQ-Q!!!!')
            A5 = torch.rand_like(state["state1"], memory_format=torch.preserve_format, 
                                 device="cpu", dtype=torch.float32)
            print('A5', A5)
            n = A5.numel()
            ...
            A5_ABSMAX = torch.zeros((blocks,), device=A5.device, dtype=torch.float32)
            A5_QMAP = state["qmap1"].clone()
            A5_deq2 = torch.zeros_like(A5, dtype=torch.float32)
            A5_deq = dequantize_blockwise(
                A5,
                absmax=A5_ABSMAX, code=A5_QMAP,
                blocksize=block_size,
            ...
            print('A5_deq init', A5_deq)
            print('A5_deq2 init', A5_deq2)
            A5_deq.mul_(beta1).add_(grad_cpu, alpha=1 - beta1)
            print('A5_deq updated', A5_deq)
            print('A5_deq', A5_deq, A5_deq.dtype, A5_deq.shape, A5_deq.device)
            quantize_blockwise(A5_deq)

            print('\nATTEMPTING QUANTIZE_BLOCKWISE NO PARAMS!!!!')
            print('EXP_AVG', exp_avg, exp_avg.dtype, exp_avg.shape, exp_avg.device)
            quantize_blockwise(exp_avg)
            print('\nATTEMPTING QUANTIZE_BLOCKWISE WITH PARAMS!!!!')
            quantize_blockwise(exp_avg, code=state["qmap1"], absmax=state["absmax1"], out=state["state1"])
            print('\nATTEMPTING QUANTIZE_BLOCKWISE 2!!!!')
