...
class CollaborativeHFTrainer(Trainer):
    ...
    def _wrap_model(self, model, training=True):
        ...
        return IgnoreGradManipulations(
            ...
            override_zero_grad=(self.collaborative_optimizer.grad_averager is not None and self.collaborative_optimizer.grad_averager.reuse_grad_buffers),
        )
...
class IgnoreGradManipulations(nn.Module):
    ...
    def zero_grad(self, set_to_none: bool = False) -> None:
        if self.override_zero_grad:
            grad_is_nan = all(param.grad.isfinite().all() for param in self.parameters() if param.grad is not None)
            if grad_is_nan:
